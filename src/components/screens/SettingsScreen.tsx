...
export default function SettingsScreen() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 px-6">
      <h1 className="font-serif text-4xl text-gold">Settings</h1>
      ...
      <div className="w-full max-w-sm">
        ...
        >
          <span className="flex items-center gap-2 text-parchment">
            {sound ? <Volume2 className="h-5 w-5" /> : <VolumeX className="h-5 w-5" />} Sound Effects
          </span>
      ...
      >
        <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" /> Back to title
      </button>
    </div>
  );
}

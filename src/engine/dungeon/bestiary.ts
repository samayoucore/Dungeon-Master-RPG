...
export function createGoblin(): Enemy {
  return spawn({
    ...
    attack: { name: 'Scimitar', toHit: 4, count: 1, dice: 'd6', bonus: 2, type: 'slashing' },
...
export function createSkeleton(): Enemy {
  return spawn({
    ...
    attack: { name: 'Bony Claw', toHit: 4, count: 1, dice: 'd6', bonus: 2, type: 'piercing' },
...
export function createZombie(): Enemy {
  return spawn({
    ...
    attack: { name: 'Slam', toHit: 3, count: 1, dice: 'd6', bonus: 1, type: 'bludgeoning' },
    behavior: 'aggressive', biomes: ['crypt', 'ruins'], xp: 50,
  });
}
...
export function createOrc(): Enemy {
  return spawn({
    ...
    attack: { name: 'Greataxe', toHit: 5, count: 1, dice: 'd12', bonus: 3, type: 'slashing' },
...
export function createDarkMage(): Enemy {
  return spawn({
    name: 'Dark Mage', cr: 1, hp: 22, ac: 12,
    stats: abilityBlock({ int: 16, dex: 14 }),
    attack: { name: 'Fire Bolt', toHit: 5, count: 3, dice: 'd6', bonus: 0, type: 'fire' },
...
export function createTroll(): Enemy {
  return spawn({
    ...
    attack: { name: 'Claw', toHit: 7, count: 2, dice: 'd6', bonus: 4, type: 'slashing' },
